...
export async function getShipLoadout(db: D1Database, slug: string): Promise<Record<string, unknown>[]> {
  ...
  // No name-variant fallback by design: CIG reuses one display name across
  // distinct variants (e.g. "MSD-683 Missile Rack" → the Asgard's ship-default
  // mrck_s07_anvl_asgard_16_s03, a 16-rack that isn't sold, vs a buyable 8-rack
  // mrck_s06_behr_octo_s03). Borrowing the same-named variant's price would
  // misrepresent a different component. A component with no shops for its exact
  // class_name is ship stock — the UI labels it "Not Sold or Lootable".
...
// ============================================================
// AI Chat — saved "Chat about my fleet" conversations (migration 0258)
// ============================================================
...
export async function createChat(
  db: D1Database,
  opts: { userId: string; provider: string; model: string; title: string },
): Promise<number> {
  const r = await db
    .prepare("INSERT INTO ai_chats (user_id, provider, model, title) VALUES (?, ?, ?, ?)")
    .bind(opts.userId, opts.provider, opts.model, opts.title)
    .run();
...
export async function addChatMessage(
  db: D1Database,
  opts: { chatId: number; role: string; content: string },
): Promise<void> {
  // Bump the parent chat's updated_at so listChats orders by recent activity.
  await db.batch([
    db
      .prepare("INSERT INTO ai_chat_messages (chat_id, role, content) VALUES (?, ?, ?)")
      .bind(opts.chatId, opts.role, opts.content),
    db.prepare("UPDATE ai_chats SET updated_at = datetime('now') WHERE id = ?").bind(opts.chatId),
  ]);
}

export async function listChats(db: D1Database, userId: string): Promise<ChatRow[]> {
  const r = await db
    .prepare(
      "SELECT id, provider, model, title, created_at, updated_at FROM ai_chats WHERE user_id = ? ORDER BY updated_at DESC",
    )
    .bind(userId)
    .all();
  return r.results as unknown as ChatRow[];
}
...
export async function getChat(
  ...
  const chat = await db
    .prepare(
      "SELECT id, provider, model, title, created_at, updated_at FROM ai_chats WHERE id = ? AND user_id = ?",
    )
    .bind(chatId, userId)
    .first<ChatRow>();
  if (!chat) return null;
  const msgs = await db
    .prepare("SELECT id, role, content, created_at FROM ai_chat_messages WHERE chat_id = ? ORDER BY id")
    .bind(chatId)
    .all();
  return { chat, messages: msgs.results as unknown as ChatMessageRow[] };
}

export async function deleteChat(db: D1Database, userId: string, chatId: number): Promise<void> {
