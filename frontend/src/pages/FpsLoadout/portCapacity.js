...
const CORE_BY_WEIGHT = {
  light: { grenades: 2, mags: 4 },
  medium: { grenades: 3, mags: 6 },
  heavy: { grenades: 4, mags: 8 },
}
...
export function portCapacity(corePiece, legsPiece) {
  const core = corePiece ? weightRow(corePiece) : { grenades: 0, mags: 0 }
  return {
    ...core,
    pens: legsPiece ? 4 : 0,
...
const FAMILY_RE = /^(grenade|mag|pen)_(\d+)$/
export function SLOT_FAMILY(slotKey) {
  ...
  const familyKey = { grenade: 'grenades', mag: 'mags', pen: 'pens' }[m[1]]
  return { family: familyKey, index: Number(m[2]) }
}

// Human labels for the dynamic utility slot-key vocabulary this module owns
// (util_gadget/util_knife singletons + the ordinal grenade_N/mag_N/pen_N
// families). Fixed structural slots (primary, helmet, ...) aren't part of
// that vocabulary — callers with their own fixed-slot labels should check
...
const FAMILY_LABEL_PREFIX = { grenades: 'Grenade', mags: 'Mag', pens: 'Pen' }
