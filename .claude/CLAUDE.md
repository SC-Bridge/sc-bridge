...
- **Stable PTU versions (0140)**: PTU/EPTU `game_versions` rows use stable codes without build numbers (e.g., `"4.7.0-ptu"` not `"4.7.0-ptu.11475995"`). Build number tracked in separate `build_number` column. User preferences survive PTU data refreshes. Admin purge endpoint (`DELETE /api/admin/versions/ptu`) wipes all PTU data from ~65 versioned tables and sets `build_number = NULL`; build update endpoint (`PUT /api/admin/versions/ptu/build`) sets the build number. Extraction scripts use `lib/version.py:resolve_version()` to produce stable PTU codes. **`getGameVersions` has no EXISTS check** — it is an unfiltered SELECT and always returned purged rows (an earlier version of this note, and a comment in `admin.ts`, both claimed otherwise; the check never existed, which is why Settings offered a stale `4.8.0-ptu` until 2026-09-25). A null `build_number` is the "purged, no data" signal and **filtering is the consumer's job** — see `frontend/src/lib/previewChannel.js`, which drives Settings → Preview Channel and hides a preview that is either purged or no longer ahead of LIVE.
